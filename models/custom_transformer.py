...
class CustomModelTransformer(BaseModel):
    ...
    def generate(self, x, attn, temperature=1.0, topk=None):        
        encoder_output = self.encoder(x, attn, output_hidden_states=True, output_attentions=False)
        ...
        for _ in range(self.MAX_LENGTH-1): # -1 to account for the SOS token
            ...
            logits_notes.append(note[:, -1, :].unsqueeze(1))
            logits_durations.append(duration[:, -1, :].unsqueeze(1))
            logits_gaps.append(gap[:, -1, :].unsqueeze(1))

            # Apply temperature
            last_note = note[:, -1, :] / temperature
            last_duration = duration[:, -1, :] / temperature
            last_gap = gap[:, -1, :] / temperature

            # Get topk
            if topk is not None:
                topk_note, _ = torch.topk(last_note, topk[0], dim=-1)
                topk_duration, _ = torch.topk(last_duration, topk[1], dim=-1)
                topk_gap, _ = torch.topk(last_gap, topk[2], dim=-1)
                last_note[last_note < topk_note[:, [-1]]] = -float('Inf')
                last_duration[last_duration < topk_duration[:, [-1]]] = -float('Inf')
                last_gap[last_gap < topk_gap[:, [-1]]] = -float('Inf')
            
            # Apply softmax
            probs_note = F.softmax(last_note, dim=-1)
            probs_duration = F.softmax(last_duration, dim=-1)
            probs_gap = F.softmax(last_gap, dim=-1)

            # Sample from the distribution
            note = torch.multinomial(probs_note, 1)
            duration = torch.multinomial(probs_duration, 1)
            gap = torch.multinomial(probs_gap, 1)
