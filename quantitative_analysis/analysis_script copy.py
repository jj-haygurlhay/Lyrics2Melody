...
with open("quantitative_analysis/rnn/gen_stats", "w") as f:
    json.dump(gen_stats, f, indent=4)
with open("quantitative_analysis/rnn/ref_stats", "w") as f:
    json.dump(ref_stats, f, indent=4)    
...
with open("quantitative_analysis/rnn/gen_clamped_stats", "w") as f:
...
def full_analysis(model_dir, ref_not, ref_dur, ref_gap, lyrics):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    topk = [30, 5, 5] # [notes, durations, gaps]
    temperature = 0.6 
    generator = Generator(model_dir, './vocab/syllables.txt', 'model_best.pt', device=device)
